...
dm_traffic = DataProcHiveOperator(
    ...
    query="""
        ...
        select user_id, min(bytes_received) as MIN_bytes, avg(bytes_received) as AVG_bytes, max(bytes_received) as MAX_bytes from pdmitry.ods_traffic where year = cast({{ execution_date.year }} as STRING);
    """,
    cluster_name='cluster-dataproc',
