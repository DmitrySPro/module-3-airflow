...
dds_hub_user = PostgresOperator(
    ...
    sql="""
        ...
        from pdmitry.ods_v_payment) as h
        where row_number=1),
...
with source_data as (
    select a.USER_PK, a.USER_HASHDIF, a.phone, a.EFFECTIVE_FROM, a.LOAD_DATE, a.RECORD_SOURCE from pdmitry.ods_v_payment as a
),
     update_records as (
...
with source_data as (
    select a.PAYMENT_PK, a.PAYMENT_HASHDIF, a.pay_date,a.sum, a.EFFECTIVE_FROM, a.LOAD_DATE, a.RECORD_SOURCE from pdmitry.ods_v_payment as a
),
     update_records as (
